...
const Dashboard: React.FC<DashboardProps> = ({ user, onLogout, onEnterRoom }) => {
  ...
  const handleAvatarChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      localStorage.setItem('studdy_user', JSON.stringify(updatedUser));
      
      const savedAccounts = JSON.parse(localStorage.getItem('studdy_accounts') || '[]');
      const updatedAccounts = savedAccounts.map((u: User) => u.id === user.id ? updatedUser : u);
      localStorage.setItem('studdy_accounts', JSON.stringify(updatedAccounts));
    };
  ...
  return (
    <div className="max-w-6xl mx-auto w-full p-6 md:p-12 animate-in slide-in-from-bottom-8 duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        >
          ...
          <div className="relative z-10 flex flex-col h-full justify-between">
            ...
            <div>
              ...
              <p className="text-indigo-100/60 text-sm font-medium">Create a new secure collaboration space.</p>
            </div>
          </div>
        </button>
        ...
        {rooms.filter(r => r.members.includes(currentUser.id)).map(room => (
          <div 
            key={room.id}
          ...
          >
            <div>
              ...
              <p className="text-slate-500 text-sm font-semibold">{room.files.length} Shared Resources</p>
            </div>
